...
class Sellers extends MY_Controller {
	...
	public function add(){
		...
		$this->form_validation->set_rules(
			'email','Email Address',
			'trim|valid_email|required|is_unique[sellers.seller_email]',
			array(
		...
		$this->form_validation->set_rules(
			...
			'required|trim|min_length[10]|max_length[10]|is_unique[sellers.user_id]',
			array(
				'required'      => '%s is required.',
				'is_unique'     => 'Your %s is already registered. Please login'
			)
		...
		if ($this->form_validation->run() == FALSE){
			...
			//Show View
			$data['main_content'] = 'admin/sellers/add_seller';
			...
			$data =  array(
				...
				'seller_mobile'	    => $this->input->post('username'),
				'seller_password'   => $password,
				'seller_company'	=> $this->input->post('company'),
                'role'              => 2
			);

			if($this->User_model->add_seller($data)){       
				$this->session->set_flashdata('seller_added', 'Seller Added');
				redirect('admin/sellers/add');
			}
		}
	}
